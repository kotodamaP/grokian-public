...
export class FileContextManager {
  ...
  constructor(
    ...
    this.mentionDropdown = new MentionDropdownController(
      ...
      this.inputEl,
      {
        ...
        getVaultFileSearchTokens: (file) => this.getVaultFileSearchTokens(file),
  ...
  private getVaultFileSearchTokens(file: TFile): string[] {
    const cache = this.app.metadataCache.getFileCache(file);
    if (!cache) {
      return [];
    }
    ...
    if (Array.isArray(aliases)) {
      tokens.push(...aliases.filter((alias): alias is string => typeof alias === 'string'));
    ...
    if (Array.isArray(cache.headings)) {
      for (const heading of cache.headings) {
        if (typeof heading.heading === 'string') {
    ...
    if (Array.isArray(cache.tags)) {
      tokens.push(...cache.tags.map(tag => tag.tag));
    }
