...
export function createInputToolbar(
  ...
  const rightControlsEl = parentEl.createDiv({ cls: 'claudian-toolbar-right-controls' });
  const modeSelector = new ModeSelector(rightControlsEl, callbacks);
  const permissionToggle = new PermissionToggle(rightControlsEl, callbacks);
