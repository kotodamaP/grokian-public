  ...
  const toolbarComponents = createInputToolbar(inputToolbar, {
    ...
    onModeChange: async (mode: string) => {
      const providerSettings = await updateTabProviderSettings(tab, plugin, (settings) => {
        const uiConfig = getTabChatUIConfig(tab, plugin);
        const capabilities = getTabCapabilities(tab, plugin);
        const currentMode = typeof settings.permissionMode === 'string'
          ? settings.permissionMode
          : 'normal';

        if (capabilities.supportsPlanMode && mode === 'plan') {
          if (currentMode !== 'plan' && tab.state.prePlanPermissionMode === null) {
            tab.state.prePlanPermissionMode = currentMode;
          }
          uiConfig.applyModeSelection?.(mode, settings);
          return;
        }

        if (capabilities.supportsPlanMode && currentMode === 'plan') {
          ...
          if (uiConfig.applyPermissionMode) {
            uiConfig.applyPermissionMode(restoreMode, settings);
          } else {
            settings.permissionMode = restoreMode;
          }
          return;
        }
      ...
      dom.inputWrapper.toggleClass(
        'claudian-input-plan-mode',
        providerSettings.permissionMode === 'plan' && getTabCapabilities(tab, plugin).supportsPlanMode,
      );
    },
