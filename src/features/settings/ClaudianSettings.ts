...
export class ClaudianSettingTab extends PluginSettingTab {
  ...
  private renderGeneralTab(container: HTMLElement): void {
    ...
    renderEnvironmentSettingsSection({
      ...
      name: this.plugin.settings.locale === 'ja' ? '共有環境' : 'Shared environment',
      desc: this.plugin.settings.locale === 'ja'
        ? '全プロバイダで共有する実行時環境変数です。PATH、プロキシ、証明書、一時フォルダなどに使います。'
