...
export class SlashCommandDropdown {
    ...
    this.filteredItems = allItems
      .filter(item => {
        const searchable = [
          ...
          item.description,
          item.displayPrefix,
          `${item.displayPrefix}${item.name}`,
          `${item.insertPrefix}${item.name}`,
          `${item.displayPrefix}${item.name}`.replace(/^\//, ''),
          `${item.insertPrefix}${item.name}`.replace(/^\//, ''),
          item.sourceLabel,
        ].filter(Boolean).join('\n').toLowerCase();
  ...
  private buildItemList(includeBuiltIns: boolean): DropdownItem[] {
    ...
    for (const entry of this.cachedProviderEntries) {
      ...
      const itemKey = `${entry.insertPrefix}${nameLower}`;
      if (seenKeys.has(itemKey) || this.hiddenCommands.has(nameLower)) {
        continue;
      }
      seenKeys.add(itemKey);
      items.push({
  ...
  private render(): void {
    ...
    if (this.filteredItems.length === 0) {
      ...
      for (let i = 0; i < this.filteredItems.length; i++) {
        ...
        if (item.sourceLabel) {
          const badgeEl = itemEl.createSpan({ cls: 'claudian-slash-badge' });
          badgeEl.setText(item.sourceLabel);
        }
