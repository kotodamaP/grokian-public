...
export class SessionStorage {
  ...
  async listMetadata(): Promise<SessionMetadata[]> {
    ...
    for (const filePath of files) {
      try {
        ...
        if (
          filePath.startsWith(`${LEGACY_SESSIONS_PATH}/`)
          || filePath.startsWith(`${LEGACY_CLAUDIAN_SESSIONS_PATH}/`)
        ) {
  ...
  private async getLoadPath(id: string): Promise<string | null> {
    ...
    const legacyClaudianFilePath = this.getLegacyClaudianMetadataPath(id);
    if (await this.adapter.exists(legacyClaudianFilePath)) {
      return legacyClaudianFilePath;
    }
  ...
  private async deleteLegacyMetadataIfPresent(id: string): Promise<void> {
    for (const legacyFilePath of [
      this.getLegacyClaudianMetadataPath(id),
      this.getLegacyMetadataPath(id),
    ]) {
      if (await this.adapter.exists(legacyFilePath)) {
  ...
  private async listUniqueMetadataFiles(): Promise<string[]> {
    ...
    const fallbackFiles = [
      ...await this.listMetadataFiles(LEGACY_CLAUDIAN_SESSIONS_PATH),
      ...await this.listMetadataFiles(LEGACY_SESSIONS_PATH),
    ];
