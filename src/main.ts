...
import { createProviderIconSvg, GROK_PROVIDER_ICON } from './shared/icons';
...
export default class ClaudianPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addRibbonIcon('grokian-logo', 'Open Grokian', () => {
      void this.activateView();
    });

    this.addCommand({
      id: 'open-view',
      name: 'Open Grokian chat view',
      callback: () => {
        void this.activateView();
      },
    });
    ...
    this.addCommand({
      id: 'initialize-story-sidecar',
      name: 'Initialize Grokian story sidecar',
      callback: async () => {
        await this.storage.storySidecar.ensureInitialized({
          projectTitle: this.getVaultDisplayName(),
        });
        new Notice('Grokian story sidecar initialized.');
      },
    });
  ...
  private registerGrokianIcon(): void {
    const activeDocument = this.app.workspace.containerEl.ownerDocument;
    const scratch = activeDocument.createElement('div');
    scratch.appendChild(createProviderIconSvg(GROK_PROVIDER_ICON, {
      className: 'grokian-ribbon-icon',
      ownerDocument: activeDocument,
    }));
    addIcon('grokian-logo', scratch.innerHTML);
  }
  ...
  private getVaultDisplayName(): string {
    try {
      return this.app.vault.getName?.() ?? 'Grokian Project';
    } catch {
      return 'Grokian Project';
    }
  }
